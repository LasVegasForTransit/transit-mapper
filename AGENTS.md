...
| An agent-assisted commit carries a `Co-Authored-By` footer                      | a diff cannot say who wrote it, so it is added while the agent is still the thing running git                            | `prepare-commit-msg` hook       |
