...
| A map surface never shows an empty backdrop where a basemap belongs             | a blank map reads as a broken product, and the bootstrap grid is a loading state rather than a destination               | `smoke:release`                 |

The basemap row is enforced by `smoke:release`, which runs in the Performance
workflow rather than in `pnpm check` — the map cannot be judged without
building the app and opening a browser, and `pnpm check` promises neither.
`apps/web/scripts/perf/map-paint.ts` holds both halves.
`assertMapSurfacePainted` runs on the editor, reader, named View and both
embeds, and fails when a canvas with correct geometry carries no content.
`assertBasemapPainted` opens the editor with no system over a local style
whose only visible content comes from a source, so nothing of ours can paint
and the assertion can only pass when the basemap itself reached the canvas.

That split matters because a surface can satisfy the first on transit content
while the basemap beneath it is blank. Both thresholds were measured against a
deliberately broken MapLibre worker rather than guessed; the numbers and the
reasoning are in that file.

The basemap is `https://tiles.openfreemap.org/styles/positron`, which needs no
key, so a blank map is never a missing credential — it is the local bootstrap
style still on screen after the remote style should have replaced it.
...
precisely so a new user never sits looking at the grid. When the gate fires,
find which stage stalled — style fetch, sprite, TileJSON, vector tile, or
paint. Two worth ruling out first: MapLibre composites only on an animation
frame, so a map in a hidden or throttled tab paints nothing while every network
request still succeeds; and MapLibre parses every source in a worker, so a
worker that failed to start leaves the style, sprite and attribution all
loading correctly over an empty canvas.
