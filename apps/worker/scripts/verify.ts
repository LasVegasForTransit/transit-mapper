...
//
// Same shape as apps/web/scripts/verify.ts on purpose: no framework, one
...
import { acceptedPreview, escapeHtmlAttribute, positiveInt, shareIdFromUrl, SHARE_ID_PATTERN } from "../src/index";
...
// --- share id shape: what reaches a database query ---
{
  ...
  check("an uppercase id is rejected — shortId never produces one", !SHARE_ID_PATTERN.test("A1B2C3"));
...
{
  check("a share URL on our origin yields its id", shareIdFromUrl(`${SITE}/s/abc123`, SITE) === "abc123");
  check("an embed URL on our origin yields its id", shareIdFromUrl(`${SITE}/e/abc123`, SITE) === "abc123");
  check("a trailing slash is tolerated", shareIdFromUrl(`${SITE}/s/abc123/`, SITE) === "abc123");
  check("another origin is refused", shareIdFromUrl("https://evil.example/s/abc123", SITE) === null);
  check("a lookalike host is refused", shareIdFromUrl("https://map.lasvegasfortransit.org.evil.example/s/abc123", SITE) === null);
  check("http against our https origin is refused", shareIdFromUrl("http://map.lasvegasfortransit.org/s/abc123", SITE) === null);
  check("a non-share path on our origin is refused", shareIdFromUrl(`${SITE}/about`, SITE) === null);
  check("a nested path under /s/ is refused", shareIdFromUrl(`${SITE}/s/abc123/extra`, SITE) === null);
  check("a malformed URL is refused rather than thrown", shareIdFromUrl("not a url", SITE) === null);
  check("an id that fails the pattern is refused even on our origin", shareIdFromUrl(`${SITE}/s/ABC!`, SITE) === null);
...
{
  check("a quote cannot close the attribute", !escapeHtmlAttribute('" onload="alert(1)').includes('"'));
  check("a tag cannot be opened", escapeHtmlAttribute("<script>") === "&lt;script&gt;");
  check("an ampersand is escaped first, so entities are not double-broken", escapeHtmlAttribute("&lt;") === "&amp;lt;");
  check("a single quote is escaped too", escapeHtmlAttribute("it's") === "it&#39;s");
  check("ordinary text is left alone", escapeHtmlAttribute("Las Vegas Line 1") === "Las Vegas Line 1");
}
...
// --- uploaded preview cards: bytes from a browser we do not control ---
{
  ...
  check("a well-formed card of exactly card size is accepted", acceptedPreview(toBase64(card)) !== null);
  check("an accepted card round-trips its bytes intact", acceptedPreview(toBase64(card))?.length === card.length);

  check("a card of the wrong size is dropped", acceptedPreview(toBase64(png(100, 100))) === null);
  check("a missing preview is dropped rather than failing the share", acceptedPreview(undefined) === null);
  check("a non-string preview is dropped", acceptedPreview({ nice: "try" }) === null);
  check("an empty string is dropped", acceptedPreview("") === null);
  check("bytes that are not a PNG are dropped", acceptedPreview(toBase64(new Uint8Array([1, 2, 3, 4, 5, 6, 7, 8]))) === null);
  check("invalid base64 is dropped rather than thrown", acceptedPreview("!!!!not base64!!!!") === null);
  ...
  const polyglot = png(PREVIEW_WIDTH, PREVIEW_HEIGHT, [...'<script>alert(1)</script>'].map((c) => c.charCodeAt(0)));
  check("a PNG with content appended after IEND is dropped", acceptedPreview(toBase64(polyglot)) === null);

  // Oversize is refused on the encoded string, before anything is allocated.
  check("an oversize payload is refused without being decoded", acceptedPreview("A".repeat(MAX_PREVIEW_BYTES * 2)) === null);
}
