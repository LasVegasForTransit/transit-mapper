...
export interface EditorState {
   ...
   *
   *  The store always holds a system, so `loading` does not mean "empty" — it
   *  means the empty one here is a placeholder that a saved document is still
   *  expected to replace. Content changes are refused while it holds, because
   *  anything drawn onto the placeholder would be destroyed by the arriving
   *  document. Everything that is not content — tool, selection, camera, the
   *  draft settings — changes freely, so the editor stays usable rather than
   *  going inert. See createEditorStore for the guard itself.
   *
   *  Distinct from `readOnly`, which is a property of the *document* (a shared
   *  snapshot nobody may edit) and which the UI reads to hide controls. This
   *  one is a property of *startup* and deliberately changes no chrome: the
   *  window it covers is measured in milliseconds, and a toolbar that appeared
   *  a moment after the page did would be worse than one that never moved. */
...
export interface CreateEditorStoreOptions {
  /** Whether a saved document is still expected to arrive. Defaults to
   *  `ready`, which is what every test, fixture, and read-only preview map
   *  wants: they hand the store its document directly and there is nothing to
   *  wait for. Only the running editor passes `loading`, because only the
   *  running editor goes looking in storage. */
  documentStatus?: DocumentStatus;
}
...
export function createEditorStore(options: CreateEditorStoreOptions = {}) {
  ...
  // True while setSystem/newSystem are installing the document itself, which is
  // the one kind of `system` change the loading guard below must let through —
  // it is what ends the wait rather than something racing it.
  ...
  const editor = createStore<EditorState>()((rawSet, get) => {
    /**
     * The one seam between every action and the store.
     *
     * While a saved document is still on its way, a change to `system` is
     * either that document arriving or something about to be destroyed by it.
     * The second kind is refused here rather than in each of the forty-odd
     * actions that can produce one, because a guard those actions have to
     * remember is a guard one of them will eventually forget.
     *
     * Refused whole rather than with the `system` key stripped out. A partial
     * that adds a way also sets `activeWayId`; keeping half of it would leave
     * the store pointing at something that was never created, which is a worse
     * state than the edit simply not happening.
     */
    const set: typeof rawSet = (partial, replace) => {
      ...
      const changesDocument =
        ...
        'system' in resolved &&
        resolved.system !== get().system;
      if (changesDocument && !applyingDocument && get().documentStatus === 'loading') {
        if (import.meta.env.DEV) {
          // Not silent. The window this covers is milliseconds long at
          // startup, so a refusal here in development almost certainly means
          // something is mutating the placeholder on a path nobody intended.
          console.warn(
      ...
      // `replace` is forwarded untouched; nothing in this store passes it, but
      // dropping it would make the wrapper quietly lossy for whoever does.
      return (rawSet as (p: typeof resolved, r?: typeof replace) => void)(resolved, replace);
    };
    ...
    return {
      ...
      setSystem: (system, opts) => {
        ...
        set({
          system,
          // Installing a document is what the wait was for, so this ends it —
          // whether the document came from storage, a shared link, or the
          // Systems dialog switching to another one.
      ...
      newSystem: () => {
        skipHistory = true;
        // Also ends the wait. Someone who asks for a blank system while one is
        // still loading has said which document they want, and the guard must
        // not then refuse the very thing they asked for.
        applyingDocument = true;
        set({
          system: createEmptySystem(),
          documentStatus: 'ready',
      ...
      setWayGeometry: (id, geometry) =>
        set((s) => {
          ...
          return {
            system: {
              ...withGeom,
              stations: reanchorStations(withGeom, id),
              updatedAt: Date.now(),
            },
          };
        }),
        ...
        set((s) => ({
          system: touch({
            ...
            medians: medians.reduce(
              (acc, m) => withComponent(acc, m.id, m.median),
              s.system.medians,
            ),
      ...
      separateCarriageways: (wayId) => {
        ...
        set((s) => {
          ...
          if (current) {
            ...
            namedWays = [
              ...s.system.namedWays,
              { id: namedWayId, name: '', wayIds: [wayId, newId] },
            ];
          }
        ...
        set((s) => {
          ...
          const updated =
            Object.keys(next).length > 0
              ? { ...bare, skippedStops: next }
              : (bare as typeof pattern);
          return {
            system: touch({
              ...s.system,
              services: s.system.services.map((sv) =>
        ...
        set((s) => {
          ...
          return {
            system: touch({
              ...
              stations: s.system.stations.map((st) =>
                st.id === stationId
                  ? { ...st, platforms: [...(st.platforms ?? []), platform] }
                  : st,
              ),
            }),
          };
      ...
      addFacility: (typeId, geometry) => {
        ...
        set((s) => {
          ...
          const host = system.stations.find(
            (st) => st.footprint && pointInPolygon(at, st.footprint),
          );
          if (host) {
            ...
            system = existing
              ...
              : {
                  ...
                  groups: [
                    ...
                    createGroupEntity(
                      [host.id, id],
                      host.name ? `${host.name} complex` : undefined,
                    ),
                  ],
                };
          }
      ...
      deleteFacility: (id) =>
        set((s) => ({
          system: touch({
            ...s.system,
            facilities: s.system.facilities.filter((f) => f.id !== id),
          }),
