  ...
  it('keeps the slower observed value for each cold and warm persistence phase', () => {
    expect(
      combineProductionPersistence({
        cold: { saveMs: 20, workerSerializationMs: 8, indexedDbWriteMs: 2 },
        warm: { saveMs: 15, workerSerializationMs: 10, indexedDbWriteMs: 1 },
      }),
    ).toEqual({ saveMs: 20, workerSerializationMs: 10, indexedDbWriteMs: 2 });
  });

  it('keeps the one production sample supplied by a partial journey', () => {
    const warm = { saveMs: 15, workerSerializationMs: 10, indexedDbWriteMs: 1 };
