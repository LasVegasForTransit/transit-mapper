...
describe('one-way affordances: draft toggle, endpoint branch, network chevrons', () => {
  ...
  describe('endpoint branch', () => {
    ...
    beforeEach(() => {
      ...
      store.commands.tools.setDraftServiceEnabled(false);
      store.commands.tools.setDraftOneWay(true);
      r = required(store.commands.ways.beginWay('road', 'straight'));
      store.commands.ways.addWayPoint(r, [-115.3, 36.1]);
      store.commands.ways.addWayPoint(r, [-115.1, 36.1]);
      store.commands.ways.finishWay();
      ...
      branchId = required(store.commands.ways.beginOneWayBranch(r, 'end'));
...
describe('station land + structures: the border IS the station; structures on its land belong to it and are real shapes', () => {
  ...
  beforeEach(() => {
    ...
    sid = required(store.commands.stations.addDrawnStation(land));
