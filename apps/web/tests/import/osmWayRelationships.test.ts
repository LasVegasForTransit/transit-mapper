...
describe('OSM import figures out how ways relate to each other at junctions and along divided streets', () => {
  describe('turn-restriction relations', () => {
    ...
    it('but no ban is placed on the bike lane', () => {
      ...
      expect(bikeNet.turnRestrictions.every((t) => !bikeLaneIds.has(t.key.split(':')[1]))).toBe(
        true,
      );
    });
  ...
  describe('carriageway pairs on divided streets', () => {
    ...
    describe('through the store', () => {
      ...
      it('carrying a median lane from the captured gap', () => {
        store.commands.network.combineCarriageways(nw.id);
        expect(
          store.getState().system.ways[0].profile.lanes.some((l) => l.kindId === 'median'),
        ).toBe(true);
      });
    });
  });
